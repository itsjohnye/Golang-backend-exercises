...
func main() {
	s := Shop{
		Verbose:        true,
		Cakes:          10,              // number of cakes to bake
		BakeTime:       5 * time.Second, // time to bake one cake
		BakeStdDev:     2 * time.Second, // standard deviation of baking time
		BakeBuf:        10,              // buffer slots between baking and icing
		NumIcers:       5,               // number of cooks doing icing
		IceTime:        6 * time.Second, // time to ice one cake
		IceStdDev:      3 * time.Second, // standard deviation of icing time
		IceBuf:         8,               // buffer slots between icing and inscribing
		InscribeTime:   6 * time.Second, // time to inscribe one cake
